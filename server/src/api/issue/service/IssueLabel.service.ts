...
export class IssueLabelService extends BaseService<IssueLabel> {
    ...
    async changeIssueLabel(issueLabelData: string[], issueId:string){
        try {
            const currentIssueAsignee = await this.issueLabel.findAll({where:{issue_id:issueId}});
            if(currentIssueAsignee.length > issueLabelData.length){
                ...
                const newItem = issueLabelData.filter((item)=> !lsCurentId.includes(item));

                if(newItem.length > 0){
                    await this.creates<CreateIssueLabelDto>(newItem.map(e=>({issue_id: issueId, label_id:e})));
                }

                return 1;
            }
        } catch (error) {
            handleResultError({message: messageUpdateFail(this.issueLabel.getTableName()),messageDetail:error})
        }
    }
}
