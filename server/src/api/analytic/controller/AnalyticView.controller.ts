...
import {
  CreateAnalyticViewDto,
  UpdateAnalyticViewDto,
} from '../dto/AnalyticView.dto';
...
export class AnalyticViewController {
  ...
  async findOverView(@Request() request: IAuthRequest) {
    return handleResultSuccess(
      await this.issueService.getOverView(request?.user?.id),
    );
  }
  ...
  @UseGuards(AuthGuard)
  async dashboardAssignedByState(
    @Request() request: IAuthRequest,
    @Query('type') typeDate: TtypeDate,
  ) {
    return handleResultSuccess(
      await this.stateService.assignedByState(request?.user?.id, typeDate),
    );
  }
  ...
  async oveView(@Request() request: IAuthRequest) {
    return handleResultSuccess(
      await this.issueService.overviewAnalytics(request.user.id),
    );
  }
  ...
  async analyticsIssueByAssigned(@Request() request: IAuthRequest) {
    return handleResultSuccess(
      await this.issueService.issuePendingByMember(request?.user?.id),
    );
  }
  ...
  async analyticstopIssueCreated(@Request() request: IAuthRequest) {
    return handleResultSuccess(
      await this.issueService.topIssueCreated(request?.user?.id),
    );
  }
  ...
  async analyticstopIssueClosed(@Request() request: IAuthRequest) {
    return handleResultSuccess(
      await this.issueService.topIssueClosed(request?.user?.id),
    );
  }
  ...
  async chartIssueClosedByMonth(@Request() request: IAuthRequest) {
    return handleResultSuccess(
      await this.issueService.chartIssueClosedByMonth(request?.user?.id),
    );
  }
  ...
  @ApiQuery({ name: 'type', enum: TypeDate, required: false })
  @UseGuards(AuthGuard)
  async dashboardAssignedByPriority(
    @Request() request: IAuthRequest,
    @Query('type') typeDate: TtypeDate,
  ) {
    return handleResultSuccess(
      await this.issueService.assignedByPriority(request?.user?.id, typeDate),
    );
  }
  ...
  update(@Param('id') id: string, @Body() analytic: UpdateAnalyticViewDto) {
    return handleResultSuccess(
      this.analyticViewService.updateById(id, analytic),
    );
  }
