...
import {
  CreateProjectPublicMemberDto,
  UpdateProjectPublicMemberDto,
} from '../dto/ProjectPublicMember.dto';
...
export class ProjectPublicMemberController {
  constructor(
    private readonly projectPublicMemberService: ProjectPublicMemberService,
  ) {}
  @Post()
  ...
  @Patch(':id')
  updateProjectPublicMember(
    @Param('id') id: string,
    @Body() project: UpdateProjectPublicMemberDto,
  ) {
    return handleResultSuccess(
      this.projectPublicMemberService.updateById(id, project),
    );
  }
