...
export class ProjectMemberController {
  ...
  @Get()
  @UseGuards(AuthGuard)
  async findOneprojects(
    @RequestNest() request: IAuthRequest,
    @Query() query: { projectId: string },
  ) {
    return handleResultSuccess(
      await this.projectMemberService.findMembers(query, request.user.id),
    );
  }
  ...
  async joinProject(
    ...
    return handleResultSuccess(
      await this.projectMemberService.joinProject(
        request.user.id,
        projectId,
        role,
      ),
  ...
  async me(
    ...
    return handleResultSuccess(
      await this.projectMemberService.me(query, request.user.id),
    );
  }
  ...
  async getProjectView(
    ...
    return handleResultSuccess(
      await this.projectMemberService.getProjectView(query, request.user.id),
    );
  }
  ...
  async updateprojectMember(
    @Param('id') id: string,
    @Body() project: UpdateProjectMemberDto,
  ) {
    return handleResultSuccess(
      await this.projectMemberService.updateById(id, project),
    );
  }
