...
import {
  CreateProjectIdentifierDto,
  UpdateProjectIdentifierDto,
} from '../dto/ProjectIdentifier.dto';
...
export class ProjectIdentifierController {
  constructor(
    private readonly projectIdentifierService: ProjectIdentifierService,
  ) {}
  @Post()
  ...
  updateProjectIdentifier(
    @Param('id') id: string,
    @Body() project: UpdateProjectIdentifierDto,
  ) {
    return handleResultSuccess(
      this.projectIdentifierService.updateById(id, project),
    );
  }
