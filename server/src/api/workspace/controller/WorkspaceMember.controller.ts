import { Controller, Get, Post, Body, Patch, Delete, Query, Param, Request as RequestNestjs, UseGuards } from '@nestjs/common';
import { ApiBearerAuth, ApiTags } from "@nestjs/swagger";
import { CreateWorkspaceMemberDtoWidthToken, UpdateWorkspaceMemberDto } from '../dto/WorkspaceMember.dto';
...
export class WorkspaceMemberController {
    ...
    async findMembersFromWorkspace(@RequestNestjs() request: IAuthRequest) {
        return handleResultSuccess(await this.workspaceMemberService.findMemberFromWorkspace(request.user.id));
    }
    ...
    @Delete(":id")
    @UseGuards(AuthGuard)
    async removeWorkspaceMember(@Param('id') id: string, @RequestNestjs() request: IAuthRequest) {
        return handleResultSuccess(await this.workspaceMemberService.removeMember(id, request.user.id));
    }
}
