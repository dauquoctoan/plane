...
export class WorkspaceMemberInviteService extends BaseService<WorkspaceMemberInvite> {
    ...
    async createsWorkspaceMemberInviteService(members: CreateWorkspaceMemberInviteDto[]) {
        ...
        try {
            const workspaeMembers = await this.workspaceMemberService.findAll({
                where: {
                    [Op.or]: members.map((e) => ({ workspace_id: e.workspace_id, '$user.email$': e.email }))
                }, include: {
                    model: User,
                    as: 'user',
                }
            ...
            if (workspaeMembers) {
                ...
                const resultUpdate = await this.workspaceMemberService.update({ is_active: true }, {
                    id: lsid
                })
                if (resultUpdate) {
                    let lsKey = {}
            ...
            if (newMembers.length > 0) {
                ...
                const lsContain = await this.findAll({
                    where: {
                        [Op.or]: newMembers.map((e) => ({ workspace_id: e.workspace_id, email: e.email }))
                    }
                });
    ...
    async getMemberInviteByEmailAndWorkspace(userId: string) {
        try {
            ...
            handleResultError({ message: messageFindFail(this.repository.getTableName()), messageDetail: 'error invalid user' })
        } catch (error) {
            handleResultError({ message: messageFindFail(this.repository.getTableName()), messageDetail: error })
        }
    }
    ...
    async joinWrokspaceInWeb(lsId: string[], userId: string) {
        try {
            const workspaceMemberInviteService =
            ...
            if (workspaceMemberInviteService) {
                const workspaceMembers = await this.workspaceMemberService.creates(workspaceMemberInviteService.map((e) => {
                    return {
                        member: userId,
            ...
            handleResultError({ message: messageCreateFail(this.repository.getTableName()), statusCode: 400, messageDetail: 'Dont create workspaceMember' })
        } catch (error) {
            handleResultError({ message: messageCreateFail(this.repository.getTableName()), statusCode: 450, messageDetail: error })
        }
    }
}
