...
export class WorkspaceMemberService extends BaseService<WorkspaceMember> {
    ...
    async findMemberFromWorkspace(userId: string) {
        try {
            ...
            handleResultError({ message: messageFindFail(this.repository.getTableName()), statusCode: 500, messageDetail: 'invalid user' })
        } catch (error) {
            handleResultError({ message: messageFindFail(this.repository.getTableName()), statusCode: 500, messageDetail: error })
        }
    }
            ...
            if (info) {
                ...
                if (requestingItem && deleteItem) {
                    if (requestingItem.role > deleteItem.role) {
                        return this.repository.update({ is_active: false }, { where: { id: deleteItem.id } })
                    } else {
                        handleResultError({ message: messageFindFail(this.repository.getTableName()), statusCode: 500, messageDetail: 'You do not have permission to remove this user' })
                    }
                }
            }
