...
import {
  ...
  UpdateCycleFavoriteDto,
} from '../dto/CycleFavorite.dto';
...
export class CycleFavoriteController {
  ...
  async create(@Body() cycleFavorite: CreateCycleFavoriteDto) {
    return handleResultSuccess(
      await this.cycleFavoriteService.create(cycleFavorite),
    );
  }
  ...
  update(
    @Param('id') id: string,
    @Body() cycleFavorite: UpdateCycleFavoriteDto,
  ) {
    return handleResultSuccess(
      this.cycleFavoriteService.updateById(id, cycleFavorite),
    );
  }
  ...
  remove(@Query('cycleId') cycleId: string, @Query('userId') userId: string) {
    return handleResultSuccess(
      this.cycleFavoriteService.remove({
        where: {
          user_id: userId,
          cycle_id: cycleId,
        },
