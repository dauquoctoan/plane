...
export class EstimatePointController {
  ...
  async create(@Body() estimatePoint: CreateEstimatePointDto) {
    return handleResultSuccess(
      await this.estimateService.create(estimatePoint),
    );
  }
  ...
  async update(
    @Param('id') id: string,
    @Body() estimatePoint: UpdateEstimatePointDto,
  ) {
    return handleResultSuccess(
      await this.estimateService.updateById(id, estimatePoint),
    );
  }
