...
class OverviewService extends BaseService {
  ...
  async getAssignedByState(type: 'week' | 'month' | 'today' | 'year' | 'all') {
    try {
      return await this.get<{ group: string; total: string }[]>(
        'analytic/dashboard-assigned-by-state?type=' + type
      );
    } catch (error) {
      console.log(error);
    }
  ...
  async getAssignedByPriority(
    type: 'week' | 'month' | 'today' | 'year' | 'all'
  ) {
    try {
      return await this.get<{ priority: string; total: string }[]>(
        'analytic/dashboard-assigned-by-priority?type=' + type
      );
    } catch (error) {
      console.log(error);
    }
