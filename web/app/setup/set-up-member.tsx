...
import { useForm, UseFormRegister, FieldValues , FieldErrors, UseFormSetValue, UseFormUnregister} from 'react-hook-form'
...
const InvitePeopleItem:React.FC<Iprops> = ({keyForm, register, errors, setValue, unregister, index, removeIndex}) =>{
 return (<div className='flex gap-3 items-center group mb-4'>
    <Input
    ...
    <IoCloseSharp 
      ...
      className={`hover:cursor-pointer ${index==0?'mt-5':''} invisible group-hover:visible`}
    />
  </div>)
}

const Member:React.FC<IPropsComponent> = ({nextStep}) => {
  ...
  return (
    <form  
      onSubmit={handleSubmit(async(data) =>{
        if(info?.workspace?.id && info.user?.id && lsKeys.length > 0){
          ...
          const result = await workspaceService.createsWorkspaceMemberInvite(lsMember);
          if(result){
            const resultInfo = await authService.upDateUser(info.user.id,{
              is_onboarded: true,
              onboarding_step:{
                profile_complete: 1,
                workspace_create: 1,
            ...
            if(resultInfo)router.push(info?.workspace?.slug || '');
          }
        }
      })}
    >
      ...
      <Button type='submit' text='submit' typeBTN='primary'/>
    </form>
  )
}
