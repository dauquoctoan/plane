...
function getStepDefault(info: IInfo): number {
  ...
  const profile_complete = Number(info?.onboarding_step?.profile_complete);
  const workspace_create = Number(info?.onboarding_step?.workspace_create);
  const workspace_invite = Number(info?.onboarding_step?.workspace_invite);
