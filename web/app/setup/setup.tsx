'use client'
import Member from "@/app/setup/set-up-member";
import Profile from "@/app/setup/set-up-profile";
import Workspace from "@/app/setup/set-up-workspace";
...
function getStepDefault(info:IInfo): number{
    const workspace_join = Number(info?.user?.onboarding_step?.workspace_join);
    const profile_complete = Number(info?.user?.onboarding_step?.profile_complete);
    const workspace_create = Number(info?.user?.onboarding_step?.workspace_create);
    const workspace_invite = Number(info?.user?.onboarding_step?.workspace_invite);
...
const Setup = () => {
    ...
    const [step, setStep] = useState<number>(getStepDefault(info||{user:null,workspace:null}));
