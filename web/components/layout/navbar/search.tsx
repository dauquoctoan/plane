...
const Search = () => {
    ...
    const {data} =  useSWR(useDebounce(search, 1000) || null, ()=>overviewService.search(search))

    const defaultDataSearch: { [e: string]: IItem[] } = {
        Issue: [...(data?.issues.map((e):IItem=>({name: e.name || '', type:'issue'}))||[]),...defaultIssue],
        Project: [...(data?.projects.map((e):IItem=>({name: e.name || '', type:'project'}))||[]),...defaultProject],
        WorkspaceSettings: [...(data?.workspaces.map((e):IItem=>({name: e.name || '', type:'workspace'}))||[]),...defaultWorkSpace],
        ...
        <div className="h-[400px] w-[700px] rounded border overflow-x-hidden flex flex-col py-1">
            <div className="border-b px-2 relative">
                ...
                <input value={search} onChange={(e)=>{
                    ...
                    }} className="placeholder:text-color-text-sidebar text-sm outline-none border-none w-full h-full pl-7 " placeholder="Change interface theme..." />
            </div>
            {search && <div className="px-2 py-2 text-xs text-color-text-sidebar">Search results for &quot;<span className='text-sm text-black'>{search}</span>&ldquo; in workspace:</div>}
            <div className="px-2 overflow-y-auto flex-1 overflow-x-hidden">
                {
                    Object.keys(defaultDataSearch).map((e) => {
                        return (
                            defaultDataSearch[e].length ? <div key={e} className='py-2'>
                                <div className="text-xs px-1">{e}</div>
                                <div>
                                    {
                                        defaultDataSearch[e].map((item, index) => {
                                            return (
                                                <div key={index} className="text-xs select-none cursor-pointer rounded px-2 py-1 hover:bg-theme-secondary flex items-center gap-2">
