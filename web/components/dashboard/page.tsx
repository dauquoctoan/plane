...
const Dashboard = () => {
    ...
    const dayAndNight = (moment().hours() <= 12 && moment().hours() >= 0) ? {
        text: 'Good morning',
    }:{
        text: 'Good afternoon',
    }
    
    const {data:overView,isLoading} = useSWR('overview',()=>overviewService.getDashboardOverview())
    return (
        <div className="w-full h-auto">
            <div>
                ...
                <span className="">🌤️ {moment().format('MMMM Do YYYY, h:mm:ss a')}</span>
            </div>
            <div>
                <Popover/>
            </div>
            ...
            <div className='grid md:grid-cols-2 gap-4 grid-cols-1'>
                ...
                <IssueConsum isTab title='Created by you' type='createBys' Content={AssignedToYou}/>
                <IssueConsum title='Assigned by state' Content={AssignedByState}/>
                <IssueConsum title='Assigned by priority' Content={AssignedByPriority}/>
            </div>
        </div>
    );
};
...
const IssueConsum:FC<IIssueConsum> = ({type, title, isTab = false, Content, defaultTab = 0, defaultDate = 1})=>{
    ...
    useEffect(()=>{
        if(query.date == selects[0].value){
            if(!curentTab.find((e)=>{e.value === query.option})){
    ...
    return (
        <div className="flex gap-5 mt-4 flex-col transition-all w-full rounded border">
                ...
                {
                    state && <Content type={type} state={state} date={query.date} option={query.option}/>
                }
        </div>
    )
}
