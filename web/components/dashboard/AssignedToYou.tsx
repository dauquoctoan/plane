      ...
      <div className="w-full">
        <div className="flex items-center justify-between px-3">
          <div className="flex items-center gap-2">
            ...
            <div className="text-[13px] text-color-special-primary px-3 py-0 rounded-2xl bg-color-special-secondary">
              {data?.length}
            </div>
          </div>
        ...
        <div className="w-full h-72 overflow-y-auto">
          {data?.map((item, index) => {
            return (
              <div
                className="flex  items-center gap-3 px-2 py-1 hover:bg-theme-secondary text-sm"
                key={index}
              >
                <div className="p-2 border rounded">
                  {optionLevelIcons[item.priority || '']}
                </div>
