...
const AutoComplete: React.FC<IProps> = ({
  ...
  useEffect(()=>{
    ...
    return ()=>{ document.removeEventListener('click', handleWhenClickBlur) }
  },[])
            ...
            <div className='relative'>
                <input
                    ...
                    onKeyDown={(e)=>{
                        ...
                        if(length > 0){
                            if(40==e.keyCode) setIndex((index + 1) > length-1 ? 0 : (index + 1))
                            if(38==e.keyCode) setIndex((index - 1) < 0 ? length-1 : (index - 1));
                            if(13==e.keyCode){
                    ...
                    id={id} 
                    className={`w-full  border ${(error && error[keyForm ?? '']) ? 'border-color-warning' : ''}  rounded outline-none p-1 ${size == 'sm' ? '' : size == 'md' ? '' : ''} ${className||''}`}
                />
                {(lsResult.length > 0 && isPopUp) ? <FiChevronUp className="absolute right-2 top-[25%] animate-pop-up"/> : <FiChevronDown className="absolute right-2 top-[25%] animate-pop-up"/>}
                {
                    (lsResult.length > 0 && isPopUp) && (<ListResult index={index} handleUpdateAndClosePopUp={handleUpdateAndClosePopUp} uuid={uuid} lsResult={lsResult}/>)
                }
            </div>
        </div>
