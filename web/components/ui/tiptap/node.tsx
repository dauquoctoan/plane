...
import { PiCheckSquareOffset, PiCodeSimple, PiListBulletsLight, PiTextAlignLeftLight, PiTextHOneLight, PiTextHThreeLight, PiTextHTwoLight } from 'react-icons/pi';
...
const Node = ({editor}:{editor:Editor}) => {
  ...
  return (
    <div  className="relative cursor-pointer flex items-center gap-1 border-r px-2">
        <button onClick={()=>{setOpen(!open)}} className='flex items-center gap-1' type="button"><span>{activeItem?.name}</span><TfiAngleDown className="text-sm"/></button>
        {
            open && 
            (
                <div className='border rounded absolute bg-theme-primary px-2 mt-2 py-1 animate-pop-up origin-top-left top-[100%] left-0 w-[200px] shadow-theme-primary'>
                    {
                        node.map((item,index)=>{
                            return (
                                <button
