...
export default function TiptapPopover(props: IProps) {
  ...
  const uploadFile = async (files: any) => {
    ...
    try {
      const file: any = await uploadService.uploadImage(formData);
      editor &&
        editor
          .chain()
          .focus()
          .setImage({
            src: APP_CONFIG.NEXT_PUBLIC_STATIC + '/images/' + file?.image_url,
          })
          .run();
    } catch (e) {
      console.error(e);
  ...
  return (
    ...
    >
      <div>
        {isHeader && (
          <div className="border-b flex justify-between px-2 py-1">
