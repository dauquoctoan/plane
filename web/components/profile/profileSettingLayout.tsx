...
const ProfileSettingLayout = () => {
    ...
    return (
        <div ref={ref} className={`md:w-auto fixed md:static z-50 top-0 left-0 ${isCollap ? 'w-auto ' : 'w-full'}  bg-color-modal-overlay  h-[100vh]`}
            onClick={(e) => {
                if (ref.current === e.target && !isCollap) dispatch(layoutSlice.actions.setToggleCollapProfileSetting(true))
            }}
        >
            <div className=''>
                <div className={`fixed transition-all ease-in-out duration-75 overflow-hidden bg-theme-primary z-50 left-0 top-0 md:static ${isCollap ? 'w-0 p-0 md:w-[300px] md:px-4 md:py-2' : 'w-[300px] px-4 py-2'} border-r h-[100vh]`}>
                    <ContainerLink
                        links={[
                            ...
                            {acounts.map((e, i) => {
                                return (
                                    <div
                                        ...
                                        className={`flex select-none items-center px-2 py-1 rounded ${path == e.href
