...
const ProfileSettingLayout = () => {
    ...
    return (
        <div className='w-[300px] px-4 py-2 border-r h-[100vh]'>
            ...
            <div className='flex group items-center gap-2 select-none cursor-pointer' onClick={() => {
                changeRoute("/" + info?.workspace?.slug)
            }}><FaAngleLeft className="group-hover:scale-110" /><div className='text-lg font-bold'>Profile settings</div></div>
            <div>
                ...
                <div className='flex flex-col gap-1'>
                    {
                        acounts.map((e) => {
                            return <div
                                ...
                                className={`flex select-none items-center px-2 py-1 rounded ${path == e.href ? 'bg-color-special-secondary text-color-special-primary' : 'hover:bg-theme-secondary'} cursor-pointer gap-2 font-semibold`}>{e.icons}<div>{e.text}</div></div>
                        })
                    }
                </div>
            </div>
            <div className='py-4 max-h-[50vh]'>
                ...
                <div className='flex flex-col gap-1 overflow-x-hidden overflow-y-auto'>
                    {
                        workspaces?.map((e) => (
                            <div className='flex gap-2 items-center px-2 py-1 cursor-pointer select-none hover:bg-theme-secondary rounded'>
                                <Avatar children={e.name || ''} />
