...
const ScopeAndDemand = () => {
  const { data } = useSWR('analytic-overview', () =>
    overviewService.getAnalyticsOverview()
  );
    ...
    <div className="w-full">
      <div className="w-full flex gap-4 md:h-[350px] flex-col md:flex-row h-[700px]">
        <div className="border flex flex-1 p-3 rounded flex-col">
          ...
          <div>
            {data?.issueAssignedByState?.reduce(
              (e, item) => e + item.total,
              0
            ) || 0}
          </div>
          <div className="flex-1 flex flex-col justify-between py-2">
            {data?.issueAssignedByState.map((stateItem, index) => {
              return (
                <ProgessBarChartItem
                  value={stateItem.total}
                  color={sateColors[stateItem.group]}
                  percent={((stateItem.total / TotalOpenTasks) * 100).toFixed(
                    0
                  )}
                  title={stateItem.group}
                  key={index}
                />
              );
            })}
            ...
            <>
              ...
              <div className="flex items-center">
                <Avatar size="sm" type="circle">
                  {data?.topIssueCreated[0].first_name +
                    ' ' +
                    data?.topIssueCreated[0].last_name}
                </Avatar>
                <div className="ml-2">
                  {data?.topIssueCreated[0].first_name +
                    ' ' +
        ...
        <div className="flex-1 p-2 border rounded">
          {data?.topIssueClosed[0] ? (
            <>
              ...
              <div className="flex items-center">
                <Avatar size="sm" type="circle">
                  {data?.topIssueClosed[0].first_name +
                    ' ' +
                    data?.topIssueClosed[0].last_name}
                </Avatar>
                <div className="ml-2">
                  {data?.topIssueClosed[0].first_name +
                    ' ' +
        ...
        <div className="flex-1">
          {data?.chartIssueClosedByMonth.length ? (
            <ResponsiveLine
              ...
              tooltip={e => {
                return (
                  <div className="px-2 rounded py-1 bg-color-special-secondary">
                    {e?.point?.data?.y + ' issues closed in ' + e.point.data.x}
                  </div>
                );
              }}
              axisLeft={null}
...
const ProgessBarChartItem: FC<IProgessBarChartItem> = ({
  title,
  color,
  percent,
}) => {
      ...
      <div>
        <div className="flex items-center justify-between">
          <div className="mb-2 flex items-center gap-2">
            ...
            <div className="rounded-xl bg-theme-secondary px-3 py-0 text-sm">
              3
            </div>
          </div>
