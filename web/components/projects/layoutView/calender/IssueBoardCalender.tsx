...
import { convertStringToDateObj, formartDate, formatDate, IItemDate } from '../../../ui/datepicker/datePicker';
...
const IssueBoardCalendar: React.FC<IProps> = ({ data }) => {
    ...
    const dataConvert = data?.reduce((sum: IItemConvertDate, item: IIssue): IItemConvertDate => {
        const keyDate = item.target_date && formartDate(convertStringToDateObj(item.target_date),FORMAT_DATE);
        if (keyDate) {
        ...
        <div className="min-h-max-body rounded shadow-theme-primary bg-theme-primary w-full h-fit flex flex-col">
            ...
            <div className="w-[200vw] md:w-full min-w-full height-table-calender h-max grid grid-cols-7 text-sm">
                {[...day,...getDate(month, year)].map((item, index) => {
                    ...
                    const isToday = !isheader && curentValue && curentValue.date == item.date && curentValue.month == item.month && curentValue?.year == item.year
                        ...
                        >
                           {
                            isheader ?(
                                <div
                                    key={index}
                                    className={`h-full select-none box-border flex justify-end px-2 w-full py-3 items-center cursor-pointer ${curentDate.getDay() == index &&
                                ...
                                <div className='w-f'>
                                    ...
                                    <div className='flex gap-2 flex-col group/item'>
                                        {
                                            keyDate && <>
                                                    <AddMoreIssue date={item} sequence={project?.identifier || ""} />
                                                    {dataConvert && dataConvert[keyDate] && dataConvert[keyDate].map((issue) => {
                                                        return <div className='border  items-center cursor-pointer rounded px-2 py-1 text-xs flex gap-2' key={issue.id}>
                                                            <div className='flex-1 text-ellipsis overflow-hidden'>{issue.name}</div>
                                                            <Popover content={<MoreToolls data={getConfigMoreIssue(issue, pathName, dispatch, noti, info )} />}>
...
const AddMoreIssue: FC<IPropsAddMore> = ({ sequence, date }) => {
        ...
        mutate(
            ...
            async (issue: any) => {
                const issueResult = await issueService.createIssue({
                    ...
                    is_draft: false,
                    priority: 'urgent',
                    target_date: moment(formartDate(date,FORMAT_DATE)).format()||undefined
                });
                if (issueResult) {
    ...
    return (
        <>
            {
                open ? (
                    <div className='border rounded py-1 px-2 flex items-center gap-2 ' ref={ref}>
                        <div className='text-xs font-semibold text-color-text-sidebar'>{sequence.toUpperCase()}</div>
                        <input autoFocus className="w-[80%] h-full text-xs border-none outline-none" onKeyDown={(e: any) => {
                            if (e.keyCode == 13) {
                                handleCreateIssue(
                                    e.target.value,
                                );
                    ...
                    (
                        <div onClick={() => { setOpen(true) }} className='w-full h-auto'>
