            ...
            </div>
            {
                cycles?.length ? (
                    <div>
                        ...
                        />
                        {cycles?.map((e, i) => (
                            <CycleItem key={i} data={e} tab={tab} />
                        ))}
                    </div>
                ) : (
                    <EmptyProjects
            ...
            <div className="flex flex-1 no-scrool items-center gap-6 overflow-x-auto scroll-hident md:justify-end">
                <div className='flex items-center gap-4'>
                    <DatePicker
                        ...
                        beforeUpdateValue={(e) => {
                            return projectService.upDateCycle(data.id, { start_date: e || null as any })
                        }}
                    ...
                    <DatePicker
                        ...
                        beforeUpdateValue={(e) => {
                            return projectService.upDateCycle(data.id, { end_date: e || null as any })
                        }}
                    >End Date</DatePicker>
                </div>
                ...
                <MdOutlineStarBorderPurple500 className={`text-lg ${data.is_favorite ? 'text-yellow-500' : ''}`}
                    onClick={async () => {
                        const result = await (
                            data.is_favorite ? projectService.deleteCyclesFavorite({ user_id: info?.id || '', cycle_id: data?.id || '' }) :
                        ...
                        if (result) {
                            mutate(CYCLES_BY_PROJECT_KEY(params.projectid + tab), (cycles: any) => {
                                return cycles.map((item: ICycle) => {
                                    return { ...item, is_favorite: data.is_favorite ? 0 : 1 }
                                })
                            })
                            noti?.success(`${data.is_favorite ? 'remove' : 'add'} cycle favorite success`)
                        }
                        else noti?.error(`${data.is_favorite ? 'remove' : 'add'} cycle favorite error`)
                    }}
                />
            </div>
            <Popover
                    content={<MoreToolls data={
                        [
                            ...
                            },
                            {
                                ...
                                icon: icons.delete,
                                cb: async () => {
                                    const result = await projectService.deleteCycle(data.id || '');
                                    if (result) {
                                        noti?.success('Delete cycle success');
                                        mutate(CYCLES_BY_PROJECT_KEY(params.projectid + tab), (e: any) => {
                                            return e.filter((itemCycle: ICycle) => (itemCycle.id != data.id))
                                        })
                                    }
