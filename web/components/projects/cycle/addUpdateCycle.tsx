...
const AddUpdateCycle: FC<IProps> = ({ id, name, handleCloseModel, defaultValues}) => {
    const {
    ...
    return (
        <form
            onSubmit={handleSubmit(async (data) => {
                ...
                if(isActive){
                    mutate(
                        CYCLES_BY_PROJECT_KEY(id + 'active'),
                        async (cycles: any) => {
        ...
        >
            <div className="flex gap-2">
                ...
                <div className="text-base font-bold select-none">{defaultValues?.id?'Update Cycle' : 'New Cycle'}</div>
            </div>
            <div className="mt-2">
                <Input
                    placeholder="Title"
            ...
            <div className="w-full flex gap-2 items-center">
                ...
                />
                <Button text={defaultValues?.id?'Update':'Create'} type="submit" typeBTN="primary" />
            </div>
        </form>
    );
};
