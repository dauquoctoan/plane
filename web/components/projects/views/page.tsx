...
const ProjectView = () => {
    ...
    const {data} = useSWR(KEY_PROJECT_VIEW(params.projectid), ()=>{
        ...
        {data?.map((item)=>{
            ...
            return (
                <div key={item.id}
                        ...
                        >
                            <ContainerLink links={[{nickname:createNickNameLink(item.id||''),href:link}]}/>
                            {item.name}
                        ...
                        <div className='flex items-center gap-2'>
                            <div className='w-5 h-5 text-xs rounded-full bg-black text-theme-primary flex items-center justify-center'>
                                {
                                    item?.creator?.username && item?.creator?.username[0].toUpperCase() || ''
                                }
                            </div>

                            <Popover content={<MoreToolls data={
                                [
                                    {
                                        text: 'Edit',
                                        icon: icons.edit,
                                    ...
                                    },
                                    {
                                        ...
                                        icon: icons.delete,
                                        cb: async ()=>{
                                            const result = await projectService.deleteProjectView(item.id||'');
                                            if(result){
                                                noti?.success('Delete project view success');
                                                mutate(KEY_PROJECT_VIEW(params.projectid),(data:any)=>{
                                                    return data.filter((itemView:IIssueViews)=>(itemView.id!=item.id))
                                                })
                                            }
