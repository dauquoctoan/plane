  ...
  const menuConfigs: IMenuConfig[] = [
    {
      ...
      render: (estimate) => {
        return (
          <Confirm
            ...
            onConfirm={async () => {
              ...
              if (result) {
                ...
                mutate<IEstimate[]>(
                  SWR_KEY_ESTIMATES(params.projectid),
                  (estimates) => {
          ...
          >
            <div className="w-full rounded cursor-pointer select-none px-2 py-1 hover:bg-theme-secondary text-xs flex items-center gap-2">
              {icons.delete}
        ...
        <div className="w-full flex flex-col gap-2 mt-2">
          {data?.map((estimate) => {
            return (
              <div
                key={estimate.id}
                className="rounded text-sm flex justify-between items-center border px-2 py-1 select-none cursor-pointer hover-scroll hover:bg-theme-secondary"
              >
                ...
                >
                  <div className="w-3 h-3 rounded bg-theme-secondary text-theme-text-primary">
                    {icons.more}
                  </div>
                </Popover>
              </div>
...
const CreateEstimates = ({
  ...
  const [countPoints, setCountPoints] = useState<number>(
    estimate_points?.length ?? 10
  );
  const {
          ...
          estimate_points: Object.keys(estimatePoints).map((key, index) => {
            return {
              value: estimatePoints[
                key as keyof Omit<TCreateEstimate, 'name' | 'description'>
              ] as string,
              key: (estimate_points?.length || 0) + index,
            };
          }),
        };
        const result = await (defaultEstimate
          ? issueService.updateEstimateByProject({
              ...estimate,
              id: defaultEstimate.id,
            })
          : issueService.createEstimate(estimate));
        if (result) {
            ...
            return (
              <div key={`id-${i}`} className="flex items-center gap-2 mb-4">
                <Input
                  wrClassName="flex-1"
                  defaultValue={
                    (estimate_points && estimate_points[i]?.value) ||
                    String(i + 1)
                  }
                  keyForm={
                    estimate_points && estimate_points[i]?.id
                      ? undefined
                      : String(i + 1)
                  }
                  error={
                    ...
                    estimate_points && estimate_points[i]?.id
                      ? async (e) => {
                          if (estimate_points && estimate_points[i]?.id) {
                            setLoading(true);
                            const result =
                              await issueService.updateEstimatePointById(
                                estimate_points[i]?.id as string,
                                  ...
                                  return data.map((estimate: IEstimate) => {
                                    if (estimate.id === defaultEstimate.id) {
                                      estimate.estimate_points =
                                        estimate.estimate_points.map(
                                          (esPoint) => {
                                            if (
                                              esPoint.id ==
                                              estimate_points[i]?.id
                                            )
                                              esPoint.value = e.target.value;
                ...
                <div
                  ...
                  onClick={async () => {
                    if (i && estimate_points && estimate_points[i]?.id) {
                      setLoading(true);
                      const result = await issueService.removeEstimatePointById(
                        estimate_points[i]?.id || ''
                      );
                      if (result) {
                          ...
                          (data: any) => {
                            return data.map((e: IEstimate) => {
                              if (e.id === defaultEstimate.id) {
                                e.estimate_points = e.estimate_points.filter(
                                  (e) => {
