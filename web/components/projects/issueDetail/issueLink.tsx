        ...
        <div className="w-full py-4">
            <Modal
                content={
                    <AddLink
                        defaultValue={
                            !openModal.isCreate ? curentItem : undefined
                        }
                        issueId={issueId}
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-2 items-center">
                                    ...
                                    <Confirm
                                        ...
                                        onConfirm={async () => {
                                            const res =
                                                await issueService.removeIssueLink(
                                                    e.id,
                                                );
                                            if (res) {
                                                noti?.success(
                                                ...
                                                mutate(
                                                    LS_ISSUE_LINK(issueId),
                                                    (prevData: any) => {
                                                        return prevData.filter(
                                                            (
                                                                itemPrevData: IIsueLink,
                                                            ) => {
                                                                return (
                                                                    itemPrevData.id !=
                                                                    e.id
        ...
        <form
            onSubmit={handleSubmit(async (data) => {
                if (!defaultValue) {
                    ...
                    const res = await issueService.updateIssueLink(
                        defaultValue.id,
                        data,
                    );
