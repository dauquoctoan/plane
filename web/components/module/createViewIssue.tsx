...
function handleCheckDataBeforeSubmit(
  ...
  if (customDate.startDate?.from || customDate.startDate?.to) {
    query.startCustomFrom = moment(customDate.startDate?.from).format() as any;

    query.startCustomTo = moment(customDate.startDate?.to).format() as any;
    delete query.startDate;
  } else {
    if (query.startDate) query.startDate = query.startDate.pop() as any;
  }
  ...
  if (customDate.dueDate?.from || customDate.dueDate?.to) {
    ...
    if (query.dueDate) query.dueDate = query.dueDate.pop() as any;
  }
...
const CreateViewIssue = ({
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-base font-bold mb-1 select-none">Create View</h2>
      </div>
            ...
            <div className="w-full h-auto flex flex-wrap gap-1">
              <ItemSelected
                ...
                handleRemoveParent={(value: string) => {
                  setItemSelected(handleRemoveParentItem(itemSelected, value));
                }}
                handleRemoveChild={(value: string) => {
                  setItemSelected(handleRemoveChildItem(itemSelected, value));
                }}
              />
            </div>
          </div>
